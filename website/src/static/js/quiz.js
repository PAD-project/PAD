...
window.ctf_quiz = async (challenge_id, challenge_flag) => {
    const quiz_info_res = await fetch(`/quiz?id=${challenge_id}&flag=${challenge_flag}`);
    ...
    Swal.mixin({
        ...
        },
        {
            ...
            preConfirm: async () => {
                const quiz_post_resp = await fetch(`/quiz?id=${challenge_id}&flag=${challenge_flag}&answer=${selected_value}`, {
                    method: 'POST'
                });
